    ...
    return new Promise(async function (resolve, reject) {
        ...
        for (const oData of aData) {
            ...
            try {
                ...
                let res = await SELECT.from("sap.ariba.ProjectTaskNameDim_AN").where(
                    {
                ...
                if (res.length == 0) {
                    //New record, insert
                    await INSERT.into("sap.ariba.ProjectTaskNameDim_AN").entries(oDataCleansed);

                } else {
                    //Update existing record
                    await UPDATE("sap.ariba.ProjectTaskNameDim_AN").set(oDataCleansed).where(
                        {
                            Realm: oDataCleansed.Realm,
                            TaskId: oDataCleansed.TaskId,
                            SourceSystem: oDataCleansed.SourceSystem
