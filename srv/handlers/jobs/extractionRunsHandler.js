    ...
    return cds.tx(async srv => {
        ...
        while (sRepeat) {
            ...
            try {
                ...
                logger.error(`Error while extracting supplier data for realm [${realm}] type [${sType}]: ${e.message}`);
                logger.warn(`Extraction run captured at page token [${oDeltaRange.pageToken}], will resume on next trigger`);
        ...
        logger.info(`Supplier Data extraction finished for realm [${realm}] - ${iProcessedEntityCount} records across ${iTotalNumOfPages} pages`);
...
async function extractSupplierQNAData(context, next) {
    return cds.tx(async srv => {
        ...
        for (const oSupplier of aSuppliers) {
            if (oSupplier.SMVendorId) {
                try {
                    ...
                    logger.error(`Failed to execute QNA extraction for supplier [${oSupplier.SMVendorId}] in realm [${sRealm}]: ${e.message}`);
                }
            }
        };
        ...
        logger.info(`QNA extraction finished for realm [${sRealm}] - ${aSuppliers.length} suppliers processed`);
...
async function extractSupplierCertificatesData(context, next) {
    return cds.tx(async srv => {
        ...
        for (const oSupplier of aSuppliers) {
            if (oSupplier.SMVendorId) {
                try {
                    ...
                    // For some reason the API often do reply with ERROR 400 for specific suppliers
                    // Those suppliers will just be logged, but the overall processing do continue
                    logger.error(`Failed to execute Certificates extraction for supplier [${oSupplier.SMVendorId}] in realm [${sRealm}]: ${e.message}`);
                }
            }
        }
        ...
        logger.info(`Certificates extraction finished for realm [${sRealm}] - ${aSuppliers.length} suppliers processed`);
...
async function extractSupplierRiskData(context, next) {
    return cds.tx(async srv => {
        ...
        for (const oSupplier of aSuppliers) {
            if (oSupplier.SMVendorId) {
                try {
                    ...
                    // For some reason the API often do reply with ERROR 400 for specific suppliers
                    // Those suppliers will just be logged, but the overall processing do continue
                    logger.error(`Failed to execute Risk extraction for supplier [${oSupplier.SMVendorId}] in realm [${sRealm}]: ${e.message}`);
                }
            }
        };
        ...
        logger.info(`Risk extraction finished for realm [${sRealm}] - ${aSuppliers.length} suppliers processed`);
        ...
        try {
            ...
            while (sRepeat) {
                ...
                try {
                    ...
                    logger.error(`Error while extracting master data for realm [${realm}] entity [${sEntityName}]: ${e.message}`);
                    logger.warn(`Extraction run captured at page token [${oDeltaRange.pageToken}], will resume on next trigger`);
            ...
            logger.info(`Master Data extraction finished for realm [${realm}] entity [${sEntityName}] - ${iProcessedEntityCount} records across ${iTotalNumOfPages} pages`);
    ...
    return cds.tx(async srv => {
        try {
            ...
            if (!sapiType || !sviewTemplateName || !realm) {
                logger.error(`Missing mandatory job parameters - realm: [${realm}], viewTemplateName: [${sviewTemplateName}], apiType: [${sapiType}]`);
                throw new Error("Missing job parameters: realm, apiType and viewTemplateName are mandatory");
            }
            ...
            while (sRepeat) {
                ...
                try {
                    ...
                    logger.error(`Error while extracting synchronous data for realm [${realm}] view [${sviewTemplateName}]: ${e.message}`);
                    logger.warn(`Extraction run captured at page token [${oDeltaRange.pageToken}], will resume on next trigger`);
            ...
            logger.info(`Synchronous Data extraction finished for realm [${realm}] view [${sviewTemplateName}] - ${iProcessedEntityCount} records across ${iTotalNumOfPages} pages`);
