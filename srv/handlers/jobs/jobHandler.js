...
async function createJob(context, next) {
    ...
    */
    ...
    if (!oDeltaRange.initialLoad && !pageToken && (!loadMode || loadMode != 'F')) {// Default behaviour delta load
        ...
        logger.info(`Delta load for Job is from ${oDeltaRange.updatedDateFrom} to ${oDeltaRange.updatedDateTo}`);
    ...
    try {
        ...
        try {
            ...
            logger.info(`Job ${createdJob.jobId} type: ${createdJob.viewTemplateName} realm: ${createdJob.Realm} apiType: ${apiType} successfully started, saving in DB`);
            ...
            try {
                ...
                if (jobDetails.pageToken && jobDetails.status.toUpperCase() == "COMPLETED") {
                    ...
                    try {
                        ...
                        logger.error(`Error while creating new job for page token ${jobDetails.pageToken}: ${e.message}`);
                    }
                }
                ...
                if (error.response && error.response.data && error.response.data.code && (error.response.data.code.toUpperCase() == "JOBEXPIRED" || error.response.data.code.toUpperCase() == "JOBNOTFOUND")) {
                    logger.warn(`Job ${job.jobId} reported ${error.response.data.code} by Ariba API, updating status to error in DB.`);
